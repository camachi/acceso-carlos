...
<body>
    ...
    <div class="seach-box">
        <form >
          ...
          <button class="search-button" onclick="searchUsers('search_query_keys.php')" type="button">Search</button>
          </div>
        </form>
      </div>
  ...
  <div id="searchResultado" class="keys-content">
    <?php 
    $sql = "SELECT * FROM llaves WHERE user_id IS NULL";
    $resultado = $conn->query($sql);
    while ($fila = $resultado->fetch_assoc())
    { 
      
      echo '<div class="keys-box">';
      echo '<form action="php/get_keys_script.php?id='.$user_id.'" method="POST">';
      ...
      echo '</div>';
      echo '<div class="area">';
      echo '<input type="hidden" name="valor" value="'.$fila["key_id"].'">';
      echo '<a class="button-fix" type="submit"><button class="boton-key">Get key</button></a>';
      echo '</form>';
      echo '</div>';
      echo '</div>';
