...
$term = $_GET['term'];
$sql = "SELECT * FROM llaves WHERE (name LIKE '%$term%' OR key_id LIKE '%$term%') AND user_id IS NULL";
...
if ($result->num_rows > 0) 
{
    ...
    { 
        ...
        echo '<input type="hidden" name="valor" value="'.$row["key_id"].'">';
        echo '<button class="boton-key">Get key</button>';
        echo '</form>';
        echo '</div>';
        echo '</div>';
    }
